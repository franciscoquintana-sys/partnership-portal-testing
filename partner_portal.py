...
EXCEL_PATH = "/Users/danielareyes/Downloads/[WIP] Source of Truth v.1.xlsx"
...
def load_connections_data():
    ...
    try:
        df = pd.read_excel(EXCEL_PATH, sheet_name="Partners")
        df2 = df[df["PROVIDER_CATEGORY"].isin(CAT_MAP.keys())].copy()
        cols = ["PROVIDER_NAME","PROVIDER_CATEGORY","COUNTRY_ISO","REGION",
                "Live/NonLive Partner/Contract signed","CONTRACT_SIGNED","URL_IMAGE"]
        uniq = df2.drop_duplicates(subset=["PROVIDER_NAME","COUNTRY_ISO"])[cols]
        conns = []
        for _, row in uniq.iterrows():
            iso2 = str(row["COUNTRY_ISO"]).strip()
            try:
                obj = pycountry.countries.get(alpha_2=iso2)
                if not obj:
                    continue
                cname, ciso3 = obj.name, obj.alpha_3
            except Exception:
                continue
            live      = str(row.get("Live/NonLive Partner/Contract signed","")).strip()
            contract  = row.get("CONTRACT_SIGNED", 0)
            if live == "Live":
                status = "Live"
            elif live == "Non Live" and contract == 1:
                status = "Contract Signed"
            elif live == "Non Live":
                status = "Non Live"
            else:
                status = "Not Started"
            url_img = str(row.get("URL_IMAGE","")).strip()
            conns.append({
                "name":        str(row["PROVIDER_NAME"]).strip(),
                "category":    CAT_MAP[str(row["PROVIDER_CATEGORY"]).strip()],
                "country":     cname,
                "country_iso3": ciso3,
                "region":      REGION_MAP.get(str(row.get("REGION","")).strip(), "Other"),
                "status":      status,
                "url_image":   url_img if url_img != "nan" else "",
            })
        return conns
    except Exception:
        return []
...
# ── Connections data (loaded from Excel) ────────────────────────────────────────
CONNECTIONS    = load_connections_data()
CONN_COUNTRIES = sorted(set(c["country"] for c in CONNECTIONS))
CONN_REGIONS   = sorted(set(c["region"]  for c in CONNECTIONS))

# Grouped countries list for the dropdown (region header → countries alphabetically)
_CONN_REGION_HEADERS = set()
CONN_COUNTRIES_GROUPED = []
for _r in CONN_REGIONS:
    _cs = sorted(set(c["country"] for c in CONNECTIONS if c["region"] == _r))
    if _cs:
        _h = f"── {_r} ──"
        _CONN_REGION_HEADERS.add(_h)
        CONN_COUNTRIES_GROUPED.append(_h)
        CONN_COUNTRIES_GROUPED.extend(_cs)
...
.topbar {
    ...
    margin-bottom: 1.5rem; color: #6E6E73; font-size: 0.875rem;
}
...
def show_connections():
    ...
    # ISO-3 codes for choropleth
    ISO3 = {
        "Argentina": "ARG", "Brazil": "BRA", "Chile": "CHL",
        "Colombia": "COL", "Ecuador": "ECU", "Germany": "DEU",
        "Guatemala": "GTM", "Mexico": "MEX", "Netherlands": "NLD",
        "Spain": "ESP", "Sweden": "SWE", "Uruguay": "URY", "USA": "USA",
    }

    # (bg, text) for chips and drill-down badges
    STATUS_CHIP = {
        "Live":             ("#DCFCE7", "#166534"),
        "Contract Signed":  ("#FFEDD5", "#7C2D12"),
        "Non Live":         ("#FEE2E2", "#991B1B"),
        "Not Started":      ("#F3F4F6", "#374151"),
    }
    STATUS_BADGE = STATUS_CHIP
    CATEGORIES   = ["Acquirers", "PSPs", "APMs"]
    ALL_STATUSES = list(STATUS_CHIP.keys())

    # ── Read current filter values from session state (widgets rendered below KPIs) ──
    sel_cats      = st.session_state.get("conn_cat",     [])
    sel_search    = st.session_state.get("conn_search",  "")
    sel_regions   = st.session_state.get("conn_region",  [])
    # Strip region headers if user accidentally clicked one
    sel_countries = [c for c in st.session_state.get("conn_country", []) if c not in _CONN_REGION_HEADERS]
    sel_statuses  = st.session_state.get("conn_status",  [])

    filtered = [
        c for c in CONNECTIONS
        if (not sel_cats      or c["category"] in sel_cats)
        and (not sel_search    or sel_search.lower() in c["name"].lower() or sel_search.lower() in c["country"].lower())
    ...
    # ── KPI cards ─────────────────────────────────────────────────────────────
    unique_names     = set(c["name"] for c in filtered)
    unique_live      = set(c["name"] for c in filtered if c["status"] == "Live")
    unique_progress  = set(c["name"] for c in filtered if c["status"] in ("Non Live", "Contract Signed"))
    unique_pipeline  = set(c["name"] for c in filtered if c["status"] == "Not Started")

    k1, k2, k3, k4 = st.columns(4)
    for col, (label, value, sub, sub_c) in zip(
        [k1, k2, k3, k4],
        [
            ("Total Connections", str(len(unique_names)),    "Unique providers",    "#6E6E73"),
            ("Live",              str(len(unique_live)),     "Fully operational",   "#10B981"),
            ("In Progress",       str(len(unique_progress)), "Active integrations", "#6E6E73"),
            ("Pipeline",          str(len(unique_pipeline)), "Under evaluation",    "#6E6E73"),
        ],
    ):
        with col:
            st.markdown(f"""
    ...
    with f1:
        st.multiselect("", CATEGORIES,     placeholder="All types",     label_visibility="collapsed", key="conn_cat")
    with f2:
        st.multiselect("", CONN_REGIONS,   placeholder="All regions",   label_visibility="collapsed", key="conn_region")
    with f3:
        st.multiselect("", CONN_COUNTRIES_GROUPED, placeholder="All countries", label_visibility="collapsed", key="conn_country")
    components.html("""<script>
    (function() {
        function styleHeaders() {
            try {
                window.parent.document.querySelectorAll('li[role="option"]').forEach(function(el) {
        ...
        new MutationObserver(styleHeaders).observe(window.parent.document.body, {childList:true, subtree:true});
        styleHeaders();
    })();
    </script>""", height=0)
    with f4:
        st.multiselect("", ALL_STATUSES,   placeholder="All statuses",  label_visibility="collapsed", key="conn_status")
    st.text_input("", placeholder="Search by provider or country...", label_visibility="collapsed", key="conn_search")

    st.markdown("""
    <div style="display:flex;gap:0.75rem;align-items:center;margin-top:0.5rem;margin-bottom:0.25rem;">
<span style="padding:0.15rem 0.6rem;border-radius:20px;background:#DCFCE7;color:#166534;font-size:0.72rem;font-weight:500;">Live</span>
        <span style="padding:0.15rem 0.6rem;border-radius:20px;background:#FFEDD5;color:#7C2D12;font-size:0.72rem;font-weight:500;">Contract Signed</span>
        <span style="padding:0.15rem 0.6rem;border-radius:20px;background:#FEE2E2;color:#991B1B;font-size:0.72rem;font-weight:500;">Non Live</span>
        <span style="padding:0.15rem 0.6rem;border-radius:20px;background:#F3F4F6;color:#374151;font-size:0.72rem;font-weight:500;">Not Started</span>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<div style='margin-top:0.75rem;'></div>", unsafe_allow_html=True)

    # ── Build per-country coverage data ──────────────────────────────────────
    cov = {}
    for conn in filtered:
        c = conn["country"]
        if c not in cov:
            cov[c] = {"live": 0, "total": 0, "cats": {"Acquirers": 0, "PSPs": 0, "APMs": 0},
                      "region": conn["region"]}
        cov[c]["total"] += 1
        if conn["status"] == "Live":
            cov[c]["live"] += 1
        cov[c]["cats"][conn["category"]] += 1

    # Build country → sorted connections lookup
    country_conns = {}
    for conn in filtered:
        c = conn["country"]
        if c not in country_conns:
            country_conns[c] = []
        country_conns[c].append(conn)
    for c in country_conns:
        country_conns[c].sort(key=lambda x: x["name"])

    drill = None

    # ── Coverage matrix ───────────────────────────────────────────────────────

    regions_present = sorted(set(d["region"] for d in cov.values()))
    ...
    <div style="display:grid;grid-template-columns:180px 1fr;
                padding:0.65rem 1.25rem;background:#F8F8FB;border-bottom:1px solid #F0F0F5;">
        <div style="font-size:0.7rem;font-weight:700;color:#8E8E93;text-transform:uppercase;letter-spacing:0.08em;">Country</div>
        <div style="font-size:0.7rem;font-weight:700;color:#8E8E93;text-transform:uppercase;letter-spacing:0.08em;">Connections</div>
    </div>""", unsafe_allow_html=True)

    rows_html = ""
    for region in regions_present:
        region_countries = sorted(
            [country for country, d in cov.items() if d["region"] == region]
        )
        # Region header row
        rows_html += f"""
        <div style="padding:0.5rem 1.25rem 0.3rem;background:#6D28D9;
                    border-bottom:1px solid #5B21B6;border-top:1px solid #5B21B6;">
            <span style="font-size:0.68rem;font-weight:700;color:#FFFFFF;
                         text-transform:uppercase;letter-spacing:0.1em;">{region}</span>
        </div>"""

        for idx, country in enumerate(region_countries):
            conns = country_conns.get(country, [])
            row_bg = "#FAFAFA" if idx % 2 == 0 else "#FFFFFF"

            chips = ""
            for conn in conns:
                bg, fg = STATUS_CHIP.get(conn["status"], ("#F3F4F6", "#374151"))
                chips += (
                    f'<span style="display:inline-flex;align-items:center;'
                    f'padding:0.18rem 0.55rem;border-radius:20px;background:{bg};'
                    f'border:1px solid {bg};font-size:0.72rem;color:{fg};'
                    f'white-space:nowrap;margin:2px;font-weight:500;">'
                    f'{conn["name"]}</span>'
                )

            rows_html += f"""
            <div style="display:grid;grid-template-columns:180px 1fr;
                        padding:0.45rem 1.25rem;background:{row_bg};
                        border-bottom:1px solid #F5F5F7;align-items:start;">
                <div style="font-size:0.82rem;font-weight:600;color:#1D1D1F;
                            padding-top:0.25rem;">{country}</div>
    ...
    # ── Provider drill-down ───────────────────────────────────────────────────
    if drill and drill in cov:
        st.markdown("<hr style='border:none;border-top:1px solid #F0F0F5;margin:0.5rem 0 1.25rem;'>",
                    unsafe_allow_html=True)
        market_conns = sorted(
            [c for c in filtered if c["country"] == drill],
            key=lambda x: (x["category"], x["name"]),
        )
        d = cov[drill]
        st.markdown(f"""
        <div style="margin-bottom:1rem;">
            <span style="font-size:1.1rem;font-weight:700;color:#1D1D1F;">{drill}</span>
            <span style="margin-left:0.75rem;font-size:0.78rem;color:#6E6E73;">
                {d['live']} live · {d['total']} total connections
            </span>
        </div>""", unsafe_allow_html=True)

        for cat in CATEGORIES:
            items = [c for c in market_conns if c["category"] == cat]
            if not items:
                continue
            st.markdown(
                f'<div style="font-size:0.72rem;font-weight:700;color:#8E8E93;text-transform:uppercase;'
                f'letter-spacing:0.09em;margin:0.75rem 0 0.6rem;">{cat}</div>',
                unsafe_allow_html=True,
            )
            cards_html = '<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:0.5rem;margin-bottom:0.5rem;">'
            for conn in items:
                bg, fg = STATUS_BADGE.get(conn["status"], ("#F3F4F6", "#4B5563"))
                logo_html = (
                    f'<img src="{conn["url_image"]}" '
                    f'style="width:28px;height:28px;object-fit:contain;border-radius:6px;" '
                    f'onerror="this.style.visibility=\'hidden\'" />'
                    if conn["url_image"] else ""
                )
                cards_html += f"""
                <div style="background:white;border-radius:12px;padding:0.75rem 0.85rem;
                    ...
                    <div style="display:flex;align-items:center;gap:0.45rem;margin-bottom:0.55rem;">
                        <div style="width:28px;height:28px;display:flex;align-items:center;
                                    justify-content:center;flex-shrink:0;">{logo_html}</div>
                        <div style="font-size:0.8rem;font-weight:600;color:#1D1D1F;
                                    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{conn['name']}</div>
                    </div>
                    <div style="display:inline-block;padding:0.18rem 0.55rem;border-radius:20px;
                                background:{bg};color:{fg};font-size:0.65rem;font-weight:600;">{conn['status']}</div>
                </div>"""
            cards_html += "</div>"
