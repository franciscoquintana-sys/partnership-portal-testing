...
// Expand selected regions + countries into the per-region / per-country
// slide block that lands right after Global Presence. Per region: one
// region overview slide. Per country (in the selected regions): one
// market-brief slide + one connections slide. Countries the presenter
// didn't pick are skipped — the region overview still lists them with a
// "Verify" tag so it's clear they exist but weren't deep-dived.
function buildRegionalBlock(regions, countries) {
  if (!regions?.length) return []
  const out = []
  for (const region of regions) {
    // Three region opener slides: tier overview, T1/T2 e-commerce metrics,
    // T1/T2 local payment methods. Order matches the source deck.
    out.push({ Component: SlideRegionTierMap, label: `${region} Tiering`, props: { region } })
    out.push({ Component: SlideRegionEcommerceMap, label: `${region} E-commerce`, props: { region } })
    out.push({ Component: SlideRegionPMsMap, label: `${region} Local PMs`, props: { region } })
    // Per-country deep dive — only for countries the presenter picked AND
    // that have processor coverage. Markets without coverage stay surfaced
    // on the tier-overview slide but don't get their own deep-dive pair.
    ...
    for (const country of countries || []) {
      ...
      out.push({ Component: SlideCountryMarket, label: `${country} Brief`, props: { region, country } })
      out.push({ Component: SlideCountryConnections, label: `${country} Coverage`, props: { region, country } })
    }
  }
  return out
}
...
function SlideViewerInner({ data, onBack, shared = false }) {
  ...
  // presentation render identically. We let the stage scale up freely when
  // there is room — fullscreen mode then renders the FULL 1920×1080
  // composition without cropping content.
  useEffect(() => {
  ...
  const { Component: SlideComponent, label: slideLabel, atmospheric, props: slideProps } = SLIDES[current]
