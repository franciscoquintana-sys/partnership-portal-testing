...
COUNTRY_DETAIL_RICH = {
    "India": {
        ...
        "regulation": {
            "Primary regulators":     "RBI (payments), MeitY (data), SEBI (securities), CCI (competition)",
            "Market entry license":   "Payment Aggregator (PA) license required since Mar 2021 for any online aggregator. Cross-border services need a separate PA-CB license (introduced 2024). Foreign players must operate via a registered Indian entity or licensed local partner.",
            "Merchant requirements":  "Indian-incorporated entity (Pvt Ltd / LLP), PAN, GST registration, current bank account in INR. Compliance with Consumer Protection (E-Commerce) Rules 2020 — mandatory disclosure of country of origin, MRP, seller details and returns policy.",
            "Data hosting":           "RBI 2018 directive: ALL payment system data must be stored only in India (no copies offshore). Personal data governed by DPDP Act 2023 — cross-border transfers permitted only to a government-approved country list.",
            "Merchant of Record":     "Allowed but constrained. Foreign MoRs require either a PA-CB license or a licensed local PA partner; GST registration is mandatory if serving Indian customers. Direct INR settlement to merchants is restricted to authorized dealers.",
            "Tokenization":           "Mandatory for card-on-file storage since Jul 2022. Only network tokens (Visa/Mastercard/RuPay) may be stored — raw PAN storage is prohibited end-to-end.",
            "Tax & withholding":      "GST 18% on MDR; TDS 1% on certain digital transactions (Sec 194-O); Equalization Levy 2% on non-resident ecommerce (being phased out under OECD Pillar One).",
            "Recurring payments":     "RBI e-mandate framework. Additional Factor of Authentication required for recurring debits above ₹15,000; pre-debit notification 24h before charge.",
            "KYC / AML":              "PMLA compliance with FATF guidelines. Aadhaar-based eKYC standard; Video KYC permitted for low-risk; risk-based ongoing diligence required.",
            "Recent changes": [
                "RBI raised UPI per-merchant limits (Apr 2026)",
                "PA-CB licenses introduced for cross-border aggregators (2024)",
                "DPDP Act 2023 in force — cross-border data transfer rules tightened",
                "Mandatory card tokenization enforced since Jul 2022",
            ],
        },
        "fx_capital": {
